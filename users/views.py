...
class MaterialsUserView(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        videos = StudyMaterialNotes.objects.filter(user=self.request.user.student.user)
        for video in videos:
        ...
        context['files'] = StudyMaterialFile.objects.filter(user=self.request.user.student.user)
        return context
